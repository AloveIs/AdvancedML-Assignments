...
def generate_data():
    ...
    for player_pair in player_pairs:
        ...
        for i in range(R):
            ...
            for j in range(M):
                choice = np.random.choice([0,1], p=[1 / float(4), 3 / float(4)])
                if choice == 1:
                    table_idx = 1 - table_idx
                player1_performance = np.random.normal(mu[table_idx, j, player_pair[0] - 1], beta)
                player2_performance = np.random.normal(mu[table_idx, j, player_pair[1] - 1], beta)
...
if __name__ == '__main__':
    ...
    for k,seq in output_sequences.iteritems():
        print k,seq
    # Save the dictionary
