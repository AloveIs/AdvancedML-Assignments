...
class Tree:
    ...
    def load_sample(self, sample):
        ...
        node.sample = sample[1]
        self.load_sample_rec(node, sample[2])

    def load_sample_rec(self, node, sample):
        """
        ...
        for child in range(0, num_children, 3):
            ...
            self.load_sample_rec(node.descendants[c], sample[child + 2])
            c = c + 1
    ...
    def print_tree(self, print_sample=False, print_parent=False, print_posterior= False):
        """
        ...
        while curr_layer != []:
            ...
            next_layer = []
            for elem in curr_layer:
                if print_parent and elem.ancestor is not None:
                    string = string + elem.name + ':' + elem.ancestor.name + ' '
                else:
                    string = string + elem.name + ' '
                if (print_sample and elem.sample != None):
                    string = string[:-1] + ': sample : ' + str(elem.sample) + " "
                if print_posterior and ("posterior" in dir(elem)):
                    string = string + ' | post : ' + str(elem.posterior)
